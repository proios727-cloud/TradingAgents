...
| `broker/` | `BrokerAdapter` interface; `RobinhoodMcpBroker` (builds exact MCP calls, gated placement); `mcp_dispatch.py` — **the one and only `mcp_call` dispatcher**, onto the `mcp__Robinhood_Trading__*` tools: allowlist-refused (unlisted/unknown tools never dispatch, not just a blocklist of known-bad ones), fail-closed, kill-switch-tripping, https/host-pinned, deadline + response-size capped, inert without an OAuth token; `PaperBroker` (in-memory, tests/dry-run). |
...
| `smoke.py` | READ-ONLY diagnostic (`python3 -m agent.cli smoke`): builds the real dispatcher/transport and validates PARSING against the live API while still inert (`armed=False, dry_run=True`) — see "Going live" step 3. Also holds `preflight(broker)` — the go/no-go check (agentic access, option level, settled cash, **and account pinning**) run before climbing to `go_live.STAGES["tiny_live"]` — see "Going live" step 3a. |
